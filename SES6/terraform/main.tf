...
resource "openstack_compute_floatingip_associate_v2" "master_ip" {
    ...
    wait_until_associated = true
    provisioner "local-exec" {
        command = "echo \"${openstack_networking_floatingip_v2.master_ip.*.address[count.index]} ${var.basename}-master.openstack.local ${var.basename}-master\" >> ${var.conf_path}/floating_ips.txt"
    }
    provisioner "local-exec" {
    ...
    provisioner "remote-exec" {
        script = "${var.remote-exec_path}/configure_nodes.sh"
    }
    provisioner "remote-exec" {
        script = "${var.remote-exec_path}/deploy_ses.sh"
    }
    connection {
...
#
# null resource to run script on monitor
# aftrer ceph-salt is finished
#
...
#    }
#    provisioner "remote-exec" {
#        script = "${var.remote-exec_path}/update_monitors.sh"
#    }
#    provisioner "remote-exec" {
#        script = "${var.remote-exec_path}/configure_ses.sh"
#    }
#    provisioner "remote-exec" {
#        script = "${var.remote-exec_path}/deploy_osds.sh"
#    }
